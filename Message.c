...
char* Message::messageAnalysis(const char* message){
	//cout << "Message::messageAnalysis()" <<endl;
	char *retMag;
	retMag = NULL;
	//解析CJSON
...
char* KeepliveMagHandler::handleRequest(cJSON *_dataitem){
	cout << "KeepliveMagHandler::handleRequest()" <<endl;
	return NULL;
}

//处理注册信息
char* RegistMagHandler::handleRequest(cJSON *_dataitem){
	cout << "RegistMagHandler::handleRequest()" <<endl;
	...
	//cout << querySql << endl;
	//判断该用户是否存在
	if(connect_pool::get_instance()->get_connect(index)->existRecord(querySql)){
		...
		connect_pool::get_instance()->get_connect(index)->insertRecord("bloguser",record);;
...
char* LoginMagHandler::handleRequest(cJSON *_dataitem){
	//cout << "LoginMagHandler::handleRequest()" <<endl;
...
char* PublishBlogMagHandler::handleRequest(cJSON *_dataitem){
	cout << "PublishBlogMagHandler::handleRequest()" <<endl;
	...
	connect_pool::get_instance()->get_connect(index)->insertRecord("blog",record);
	
	//获取blog_id字段的最大值
	string condition = "where user_id=";
	condition += std::to_string(userid->valueint);
	maxBlogEditId = connect_pool::get_instance()->get_connect(index)->getMaxValue("blog","blog_id",condition);
	if(maxBlogEditId == -1){
...
char *UpdateNoteHandler::handleRequest(cJSON *_dataitem){
	cout << "UpdateNoteHandler::handleRequest()" <<endl;
	cJSON *userid = cJSON_GetObjectItem(_dataitem,"userid");
	...
	connect_pool::get_instance()->get_connect(index)->updateRecord("blog",record,string(condition));
...
char *RecommendBlogMagHandler::handleRequest(cJSON *_dataitem){
	cout << "RecommendBlogMagHandler::handleRequest()" <<endl;
	cJSON *userid = cJSON_GetObjectItem(_dataitem,"userid");
